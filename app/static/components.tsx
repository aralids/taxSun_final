...
let labelThreshold = 1;
let divLabelsArr: string[] = [];
...
function copy2clipboard(copyAll: boolean = false): void {
	...
	if (!allTaxaReduced[name]["successfulIndices"]) {
		...
		seqIDsArray = allTaxaReduced[name]["successfulIndices"].map(
			(item) => allTaxaReduced[name]["geneNames"][item]
		);
	}
	...
	if (copyAll) {
		for (let child of allTaxaReduced[name]["descendants"]) {
			if (!allTaxaReduced[child]["successfulIndices"]) {
				...
				seqIDsArray = seqIDsArray.concat(
					allTaxaReduced[child]["successfulIndices"].map(
						(item) => allTaxaReduced[child]["geneNames"][item]
					)
				);
			}
		}
...
document.getElementById("download-seq")!.addEventListener("click", () => {
	...
	let seqIDsArray: string[] = [];
	if (!allTaxaReduced[name]["successfulIndices"]) {
		seqIDsArray = allTaxaReduced[name]["fastaHeaders"].map((item, index) => [
			item,
			allTaxaReduced[name]["eValues"][index],
			name,
			findRealName(index, allTaxaReduced[name]["names"], name),
		]);
	} else {
		seqIDsArray = allTaxaReduced[name]["successfulIndices"].map((item) => [
			allTaxaReduced[name]["fastaHeaders"][item],
			allTaxaReduced[name]["eValues"][item],
			name,
			findRealName(item, allTaxaReduced[name]["names"], name),
		]);
	}
	let seqsArray = seqIDsArray.map((item) => {
		if (!headerSeqObject[item[0]]) {
			...
			let thirdElement: string =
				item[2] === item[3] ? item[2] : `${item[2]} (${item[3]})`;
...
document.getElementById("download-all-seq")!.addEventListener("click", () => {
	...
	for (let child of allTaxaReduced[name]["descendants"]) {
		if (!allTaxaReduced[child]["successfulIndices"]) {
			seqIDsArray = seqIDsArray.concat(
				allTaxaReduced[child]["fastaHeaders"].map((item, index) => [
					item,
					allTaxaReduced[child]["eValues"][index],
					child,
					findRealName(index, allTaxaReduced[child]["names"], child),
				])
			);
		} else {
			seqIDsArray = seqIDsArray.concat(
				allTaxaReduced[child]["successfulIndices"].map((item) => [
					allTaxaReduced[child]["fastaHeaders"][item],
					allTaxaReduced[child]["eValues"][item],
					child,
					findRealName(item, allTaxaReduced[child]["names"], child),
				])
			);
		}
	}
...
document.getElementById("file")?.addEventListener("change", () => {
	...
	$.ajax({
		...
		success: function (response) {
			...
			originalAllTaxaReduced = JSON.parse(
				JSON.stringify(response["allTaxaReduced"])
			);
...
document.addEventListener("mousemove", (e) => {
	...
	if (
		!target.classList.contains("hoverable-object") &&
		!target.classList.contains("taxsun-interactee")
	) {
...
class PlotDrawing extends React.Component<
	{ lineages: string[][]; ranks: string[][] },
	{
		root: string;
		layer: number;
		collapse: boolean;
		...
		taxonSpecifics: object;
		croppedLineages: string[][];
		alignedCroppedLineages: string[][];
		croppedRanks: string[][];
		unassignedCounts: string[][];
		structureByDegrees: object;
...
> {
	...
	componentDidMount() {
		...
		document.getElementById("new-data")!.addEventListener("change", () => {
			...
			let currentAlteration: any = document.querySelector(
	...
	cropLineages(
		...
		alteration = this.state.alteration,
		collapse = this.state.collapse,
		plotEValue = this.state.plotEValue,
		lineages = lineagesNames,
		ranks = lineagesRanks
	): void {
		...
		if (plotEValue) {
			currPlotId =
				fileName +
				originalAllTaxaReduced["root"]["totalCount"] +
				root +
				layer +
				collapse +
				alteration +
				...
				eThreshold +
				" " +
				viewportDimensions["cx"] +
				viewportDimensions["cy"];
		} else {
			currPlotId =
				fileName +
				originalAllTaxaReduced["root"]["totalCount"] +
		...
		if (croppedLineages[0]) {
			...
			ancestorRanks = croppedRanks[0].slice(0, layer);
		}
		...
		let ranksUnique: string[] = croppedRanks.reduce(
		...
		croppedLineages = croppedLineages.map((item) => {
			item.splice(0, 1, root);
			return item;
		});
		...
		for (let i = 0; i < croppedLineages.length; i++) {
			...
			taxonSpecifics[taxName]["rank"] =
				croppedRanks[i][croppedRanks[i].length - 1];
			taxonSpecifics[taxName]["croppedLineage"] = croppedLineages[i];
			taxonSpecifics[taxName]["alignedCroppedLineage"] =
				alignedCroppedLineages[i];
			taxonSpecifics[taxName]["firstLayerUnaligned"] =
				croppedLineages[i].length - 1;
			taxonSpecifics[taxName]["firstLayerAligned"] =
				alignedCroppedLineages[i].indexOf(taxName);
			if (changedLineages[i] || taxName.includes("&")) {
				let taxa: string[] = taxName.split(" & ");
				let unassignedCount: number = taxa
					.map((item) => allTaxaReduced[item]["totalCount"])
				...
				taxonSpecifics[taxName]["unassignedCount"] =
					allTaxaReduced[taxName].unassignedCount;
				taxonSpecifics[taxName]["totalCount"] =
					allTaxaReduced[taxName]["totalCount"];
			}
		}
		...
		for (let i = 0; i < croppedLineages.length; i++) {
			for (let j = croppedLineages[i].length - 2; j >= 0; j--) {
				if (!taxonSpecifics[croppedLineages[i][j]]) {
					...
					taxonSpecifics[croppedLineages[i][j]]["croppedLineage"] =
						croppedLineages[i].slice(0, j + 1);
					let index = alignedCroppedLineages[i].indexOf(croppedLineages[i][j]);
					taxonSpecifics[croppedLineages[i][j]]["alignedCroppedLineage"] =
						alignedCroppedLineages[i].slice(0, index + 1);
					taxonSpecifics[croppedLineages[i][j]]["unassignedCount"] = 0;
					taxonSpecifics[croppedLineages[i][j]]["totalCount"] =
						allTaxaReduced[croppedLineages[i][j]]["totalCount"];
		...
		if (croppedLineages.length >= 1) {
			this.assignDegrees({
				...
				taxonSpecifics: taxonSpecifics,
				croppedLineages: croppedLineages,
				alignedCroppedLineages: alignedCroppedLineages,
				ancestors: ancestors,
	...
	collapse(
		...
		for (let i = 0; i < layers.length - 1; i++) {
			for (let j = 0; j < layers[i].length; j++) {
				if (
					layers[i].filter((item) => item === layers[i][j]).length === 1 &&
					Boolean(layers[i + 1][j])
				) {
		...
		for (let i = 0; i < lineagesCopy.length; i++) {
			lineagesCopy[i] = lineagesCopy[i].filter(
	...
	filterByEValue(croppedLineages: string[][], croppedRanks: string[][]): any {
		...
		for (let i = croppedLineages.length - 1; i >= 0; i--) {
			...
			let oldUnassignedCount: number =
				originalAllTaxaReduced[lastTaxon]["unassignedCount"];
			...
			let eValues = originalAllTaxaReduced[lastTaxon]["eValues"].filter(
				(item, index) => {
					if (item <= eThreshold) {
			...
			allEValues = allEValues.concat(
				originalAllTaxaReduced[lastTaxon]["eValues"]
			);
		}
	...
	marryTaxa(
		...
		for (let lineage of croppedLineages) {
			totalUnassignedCounts +=
				allTaxaReduced[lineage[lineage.length - 1]]["unassignedCount"];
		}
		...
		for (let lineage of croppedLineages) {
			...
			if (
				allTaxaReduced[lineage[lineage.length - 1]]["totalCount"] /
					totalUnassignedCounts <
				threshold
			) {
				...
				for (let i = lineage.length - 2; i >= 0; i--) {
					if (
						allTaxaReduced[lineage[i]]["totalCount"] / totalUnassignedCounts >=
						threshold
					) {
		...
		if (alteration === "marriedTaxaI") {
			for (let item of reducibleLineages) {
				if (!reductionGroups[item[1].join("")]) {
					...
					reductionGroups[item[1].join("")]["commonName"] =
						croppedLineages[item[0]][item[1].length];
				} else {
					reductionGroups[item[1].join("")]["index"].push(item[0]);
					let taxa: string =
						reductionGroups[item[1].join("")]["commonName"].split(" & ");
		...
		// same ancestry, they will be combined into another wedge.
			...
			for (let group of Object.keys(reductionGroups)) {
				...
				reductionGroups[group]["index"].sort(
					(index1, index2) =>
						allTaxaReduced[croppedLineages[index1][spliceAt]]["totalCount"] -
						allTaxaReduced[croppedLineages[index2][spliceAt]]["totalCount"]
				);
				...
				let renameables: string[] = reductionGroups[group]["index"].map(
					(item) => croppedLineages[item][spliceAt]
				);
				...
				for (let i = 0; i < renameables.length; i++) {
					...
					if (!temporaryObject[renameable]) {
						...
						temporaryObject[renameable].push(
							reductionGroups[group]["index"][i]
						);
					}
				}
				...
				reductionGroups[group]["minimalIndexArray"] = Object.keys(
					permanentObject
				).sort((index1, index2) => {
					return (
						allTaxaReduced[croppedLineages[index1][spliceAt]]["totalCount"] -
			...
			for (let group of Object.keys(reductionGroups)) {
				let minimalIndexArray: number[] = reductionGroups[group][
					"minimalIndexArray"
				...
				let iterations: number =
					minimalIndexArray.length % 2 === 0
						? minimalIndexArray.length / 2
						: Math.floor(minimalIndexArray.length / 2) + 1;
				let spliceAt: number = reductionGroups[group]["spliceAt"];
				...
				while (
					(minimalIndexArray.length % 2 === 0 &&
						indexBeginning <= iterations &&
						minimalIndexArray.length - 1 - indexEnd < iterations) ||
					(minimalIndexArray.length % 2 === 1 &&
						indexBeginning !== iterations &&
						minimalIndexArray.length - 1 - indexEnd < iterations)
				) {
					if (addNext === "indexBeginning") {
						...
						let totalCount: number =
							allTaxaReduced[croppedLineages[newIndex][spliceAt]]["totalCount"];
				...
				newGroups = newGroups.map((item) =>
					item.map((item1) => reductionGroups[group]["references"][item1])
				);
				newGroups = newGroups.map((item) =>
					item.reduce((accumulator, value) => accumulator.concat(value), [])
				);
			...
			for (let group of Object.keys(reductionGroups)) {
				for (let i = 0; i < reductionGroups[group]["newGroups"].length; i++) {
					...
					newReductionGroups[`${group}-${i}`]["spliceAt"] =
						reductionGroups[group]["spliceAt"];
					newReductionGroups[`${group}-${i}`]["index"] =
						reductionGroups[group]["newGroups"][i];
					var names: string[] = reductionGroups[group]["newGroups"][i]
						.map(
							(item) =>
								croppedLineages[item][reductionGroups[group]["spliceAt"]]
						)
		...
		for (let group of Object.keys(reductionGroups).filter(
			(item) => reductionGroups[item]["index"].length > 1
		)) {
			for (let index of reductionGroups[group]["index"]) {
				croppedLineages[index].splice(
					reductionGroups[group]["spliceAt"],
					croppedLineages[index].length - reductionGroups[group]["spliceAt"],
					reductionGroups[group]["commonName"]
				);
		...
		for (let i = croppedLineages.length - 1; i >= 0; i--) {
			let croppedLineageCopy = croppedLineages.map((item) =>
				JSON.stringify(item)
			);
	...
	assignDegrees(newState: object): void {
		...
		if (newState["alteration"] === "allEqual") {
			for (let taxName of Object.keys(taxonSpecifics).filter(
				(item) => taxonSpecifics[item]["unassignedCount"] !== 0
			)) {
		...
		for (let i = 0; i < croppedLineages.length; i++) {
			for (let j = 0; j < croppedLineages[i].length; j++) {
				...
				let alignedIndex: number =
					taxonSpecifics[currentTaxon]["firstLayerAligned"];
				...
				if (!ranges[currentTaxon]) {
					...
					let firstLayer: number =
						taxonSpecifics[currentTaxon]["firstLayerUnaligned"] === 1
							? 1
							: alignedIndex;
				...
				if (j === croppedLineages[i].length - 1) {
					...
					lastLayer = alignedCroppedLineages[i].indexOf(
						croppedLineages[i][j + 1]
					);
				}

				ranges[currentTaxon]["layers"].push(lastLayer);
				ranges[currentTaxon]["degrees"].push(
					startDeg +
						(taxonSpecifics[croppedLineages[i][croppedLineages[i].length - 1]][
							"unassignedCount"
						] *
							360) /
							totalUnassignedCounts
	...
	calculateSVGPaths(newState: object): void {
		...
		let layerWidth: number = Math.max(
			viewportDimensions["smallerDimSize"] / numberOfLayers,
			dpmm * 1
		...
		for (let key of Object.keys(taxonSpecifics)) {
			...
			if (layers[0] === 0) {
				taxonSpecifics[key][
					"svgPath"
				] = `M ${cx}, ${cy} m -${layerWidth}, 0 a ${layerWidth},${layerWidth} 0 1,0 ${
					layerWidth * 2
			...
			// If the shape to be drawn is NOT the center of the plot, but a complex shape.
				...
				if (round(endDeg - startDeg) === 360) {
					let innerArcPath: string = `M ${cx}, ${cy} m -${innRad}, 0 a ${innRad},${innRad} 0 1,0 ${
						innRad * 2
					...
					// ...and is of irregular shape.
						...
						let lineInnertoOuter = `L ${midArc["x1"]},${midArc["y1"]} ${cx},${
							cy + layers[layers.length - 1] * layerWidth
						}`;
	...
	calculateTaxonLabels(newState: object): void {
		...
		let layerWidthInPx: number = Math.max(
			viewportDimensions["smallerDimSize"] / numberOfLayers,
			viewportDimensions["dpmm"] * 1
		...
		for (let key of Object.keys(taxonSpecifics)) {
			...
			let endDeg =
				taxonSpecifics[key]["degrees"][
					taxonSpecifics[key]["degrees"].length - 1
				];
			let centerDegree = startDeg + (endDeg - startDeg) / 2;
			let centerRadius = taxonSpecifics[key]["firstLayerAligned"] + 0.333;

			let centerX =
				round(centerRadius * layerWidthInPx * cos(centerDegree)) + cx;
			let centerY =
				round(-centerRadius * layerWidthInPx * sin(centerDegree)) + cy;
			let pointOnLeftBorderX =
				round(centerRadius * layerWidthInPx * cos(startDeg)) + cx;
			let pointOnLeftBorderY =
				round(-centerRadius * layerWidthInPx * sin(startDeg)) + cy;
			let pointOnRightBorderX =
				round(centerRadius * layerWidthInPx * cos(endDeg)) + cx;
			let pointOnRightBorderY =
				round(-centerRadius * layerWidthInPx * sin(endDeg)) + cy;
		...
		for (let key of Object.keys(taxonSpecifics)) {
			...
			if (taxonSpecifics[key]["layers"][0] === 0) {
				taxonSpecifics[key]["label"] = {
					...
					hoverDisplay: "unset",
					angle: 0,
					abbreviation: root.replace(
						RegExp(rankPatternFull.map((item) => " " + item).join("|"), "g"),
						""
					),
					display: "unset",
					fullLabel: root.replace(
						RegExp(rankPatternFull.map((item) => " " + item).join("|"), "g"),
						""
					),
				};
				...
				let direction =
					taxonSpecifics[key]["layers"].length === 2 &&
					taxonSpecifics[key]["layers"][1] === numberOfLayers
						? "radial"
						: "verse";
				...
				if (direction === "radial") {
					...
					angle =
						(270 - centerDeg + 360) % 360 > 180 &&
						(270 - centerDeg + 360) % 360 <= 360
							? centerDeg % 360
							: (centerDeg + 180) % 360;
					radialAngle = centerDeg <= 180 ? -centerDeg : centerDeg;
				}

				let percentage: number = round(
					(taxonSpecifics[key]["totalCount"] / totalUnassignedCount) * 100
				);
				taxonSpecifics[key]["label"] = {
					direction: direction,
					opacity: "1",
				...
				if (taxonSpecifics[key]["rank"] === "species") {
					...
					if (abbr.split(" ").length >= 2 && !(abbr.split(" ")[1] === "sp.")) {
						let newAbbr: string = (
							abbr.split(" ")[0].slice(0, 1) +
							". " +
							abbr.split(" ").slice(1, abbr.split(" ").length).join(" ")
						).slice(0, 15);
						taxonSpecifics[key]["label"]["abbreviation"] = newAbbr;
					} else if (abbr.split(" ").indexOf("sp.") !== -1) {
						let newAbbr: string = abbr
							.split(" ")
							.slice(0, abbr.split(" ").indexOf("sp.") + 1)
							.join(" ")
							.slice(0, 15);
						taxonSpecifics[key]["label"]["abbreviation"] = newAbbr;
					}
	...
	calcShapeColors(newState: object): void {
		var croppedLineages: string[][] =
			newState["croppedLineages"] == undefined
				? this.state.croppedLineages
				: newState["croppedLineages"];
		var croppedLineages: string[][] = JSON.parse(
			JSON.stringify(croppedLineages)
		);
		var taxonSpecifics =
			newState["taxonSpecifics"] == undefined
				? this.state.taxonSpecifics
				: newState["taxonSpecifics"];
		...
		for (let i = 0; i < croppedLineages.length; i++) {
			if (croppedLineages[i].length > 1) {
				...
				if (strokes.indexOf(firstAncestor) === -1) {
					taxonSpecifics[firstAncestor]["fill"] =
				...
				for (let j = 2; j < croppedLineages[i].length; j++) {
					...
					let nextColorIndex =
						(colors.indexOf(ancestorColor) + 1) % colors.length;
					let nextColor = colors[nextColorIndex];
					let selfStartDeg: number =
						taxonSpecifics[croppedLineages[i][j]]["degrees"][0];
					let ancestorStartDeg: number =
						taxonSpecifics[croppedLineages[i][1]]["degrees"][0];
					let ancestorEndDeg: number =
						taxonSpecifics[croppedLineages[i][1]]["degrees"][
							taxonSpecifics[croppedLineages[i][1]]["degrees"].length - 1
						];
					let coef: number =
						(selfStartDeg - ancestorStartDeg) /
						(ancestorEndDeg - ancestorStartDeg);
					let tintFactor: number =
						(taxonSpecifics[croppedLineages[i][j]]["firstLayerAligned"] - 1) /
						10;
	...
	placeLabels(alreadyRepeated: boolean = this.state.alreadyRepeated) {
		let newTaxonSpecifics: object = JSON.parse(
			JSON.stringify(this.state.taxonSpecifics)
		);
		let height: number = 0;
		for (let key of Object.keys(newTaxonSpecifics)) {
			if (!this.state.taxonSpecifics[key]["label"]["inDiv"]) {
				...
				let labelElement: any = document.getElementById(
					`${key}_-_${newTaxonSpecifics[key]["firstLayerUnaligned"]}-label`
				)!;
				let hoverLabelElement: any = document.getElementById(
					`${key}_-_${newTaxonSpecifics[key]["firstLayerUnaligned"]}-hoverLabel`
				)!;
				height = !alreadyRepeated
					? labelElement.getBoundingClientRect().height / 2
					: this.state.height;
				...
				if (label["direction"] === "radial") {
					...
					let sliceHere: number = round(
						(((this.state.numberOfLayers -
							newTaxonSpecifics[key]["firstLayerAligned"] +
							1) *
							this.state.layerWidth) /
							viewportDimensions["2vmin"]) *
							2.3
					);
				...
				// For internal wedges, calculate:
					...
					let fourPoints = getFourCorners(
						topBeforeRotation,
						bottomBeforeRotation,
						leftBeforeRotation,
						rightBeforeRotation,
						center[0],
						center[1],
					...
					let layersCopy = JSON.parse(
						JSON.stringify(newTaxonSpecifics[key]["layers"])
					);
					let lastViableLayer = layersCopy.sort(function (a, b) {
						return a - b;
					})[1];
					let pointOnLastLayerX =
						lastViableLayer * this.state.layerWidth * cos(center[2]);
					pointOnLastLayerX =
						round(pointOnLastLayerX) + viewportDimensions["cx"];
					let pointOnLastLayerY =
						-lastViableLayer * this.state.layerWidth * sin(center[2]);
					pointOnLastLayerY =
						round(pointOnLastLayerY) + viewportDimensions["cy"];
					let verticalSpace = lineLength(
						center[0],
						center[1],
						pointOnLastLayerX,
						pointOnLastLayerY
					...
					if (verticalSpace > horizontalSpace) {
						...
						howManyLettersFit =
							Math.floor(horizontalSpace / lengthPerLetter) - 2 > 0
								? Math.floor(horizontalSpace / lengthPerLetter) - 2
								: 0;
					}
				...
				// Calculations for root shape in the center.
					...
					let lengthPerLetter = width / label["abbreviation"].length;
					howManyLettersFit =
						Math.floor((this.state.layerWidth * 2) / lengthPerLetter) - 2 > 0
							? Math.floor((this.state.layerWidth * 2) / lengthPerLetter) - 2
							: 0;
					abbreviation = label["abbreviation"].slice(0, howManyLettersFit);
				}
				...
				abbreviation =
					abbreviation.indexOf(".") >= 0 ||
					!label["fullLabel"].split(" ")[0][howManyLettersFit]
						? abbreviation
						: abbreviation + ".";
				...
				let fourPoints = getFourCorners(
					...
					left - 2,
					left + width + 2,
					newTaxonSpecifics[key]["center"][0],
					newTaxonSpecifics[key]["center"][1],
					angle
				);
				...
				let shape: any = document.getElementById(
					`${key}_-_${this.state.taxonSpecifics[key]["firstLayerUnaligned"]}`
				)!;

				if (
					alreadyRepeated &&
					(label["direction"] === "verse" ||
						label["direction"] === "horizontal")
				) {
					if (
						!(
							shape.isPointInFill(bottomLeft) &&
							shape.isPointInFill(topLeft) &&
							shape.isPointInFill(bottomRight) &&
							shape.isPointInFill(topRight)
						)
					) {
						label["display"] = "none";
					}
				...
				label["left"] = left;
				label["transform"] = !alreadyRepeated
					? `rotate(0)`
					: `rotate(${angle} ${transformOrigin})`;
		...
		if (!alreadyRepeated) {
			this.setState({
				taxonSpecifics: newTaxonSpecifics,
	...
	handleClick(shapeId, e): void {
		let taxon: string =
			typeof e.detail === "object"
				? e.detail.taxName
				: e.target.getAttribute("data-taxname");
		let rank: string =
			typeof e.detail === "object"
				? e.detail.taxRank
				: e.target.getAttribute("data-taxrank");
		let nextLayer;
		if (taxon.includes("&")) {
			nextLayer =
				originalAllTaxaReduced[taxon.split(" & ")[0]]["lineageNames"].length -
				1;
		} else {
	...
	render() {
		...
		if (
			Object.keys(alreadyVisited).indexOf(plotId) === -1 &&
			this.state.numberOfLayers !== -1
		) {
		...
		for (let item of tSkeys) {
			...
			let redirectTo: string =
				tS[item]["layers"][0] === 0
					? `${this.state.ancestors[this.state.ancestors.length - 1]}_-_0`
					: id;
			...
			shapes.push(
				<TaxonShape
					key={id}
					id={id}
					abbr={tS[item]["label"]["abbreviation"]}
					taxName={redirectTo.split("_-_")[0]}
					taxRank={tS[item]["rank"]}
					onClick={(event) => this.handleClick(redirectTo, event)}
					d={tS[item]["svgPath"]}
					onContextMenu={(e) => {
						showContextMenu(e);
					}}
					strokeWidth={viewportDimensions["dpmm"] * 0.265}
					fillColor={tS[item]["fill"]}
					labelOpacity={tS[item]["label"]["opacity"]}
					labelDisplay={tS[item]["label"]["display"]}
					fullLabel={tS[item]["label"]["fullLabel"]}
					stroke={tS[item]["stroke"]}
					transformOrigin={tS[item]["label"]["transformOrigin"]}
					root={this.state.root}
				/>
			);
			if (tS[item]["married"]) {
		...
		for (let item of tSkeys) {
			...
			let label = (
				<TaxonLabel
					...
					taxName={redirectTo.split("_-_")[0]}
					taxRank={tS[item]["rank"]}
					transform={tS[item]["label"]["transform"]}
					left={tS[item]["label"]["left"]}
					top={tS[item]["label"]["top"]}
					opacity={tS[item]["label"]["opacity"]}
					labelDisplay={tS[item]["label"]["display"]}
					display={tS[item]["label"]["display"]}
					onClick={(event) => {
						this.handleClick(redirectTo, event);
					}}
					fullLabel={tS[item]["label"]["fullLabel"]}
					fontWeight="normal"
					root={this.state.root}
		...
		for (let item of tSkeys) {
			...
			let labelBackground = (
				<LabelBackground
					key={`${id}-labelBackground`}
					id={`${id}-labelBackground`}
					taxName={redirectTo.split("_-_")[0]}
					taxRank={tS[item]["rank"]}
					transform={tS[item]["label"]["transform"]}
					left={tS[item]["label"]["hoverLeft"] - 4}
					top={tS[item]["label"]["top"] - this.state.height - 4}
					selfDisplay="none"
					labelDisplay={tS[item]["label"]["display"]}
					onClick={(event) => {
						this.handleClick(redirectTo, event);
					...
					width={tS[item]["label"]["hoverWidth"] + 8}
					stroke="#800080"
					fill="#ffffff"
					root={this.state.root}
				/>
			...
			let hoverLabel = (
				<TaxonLabel
					key={`${id}-hoverLabel`}
					id={`${id}-hoverLabel`}
					abbr={tS[item]["label"]["fullLabel"]}
					taxName={redirectTo.split("_-_")[0]}
					taxRank={tS[item]["rank"]}
					transform={tS[item]["label"]["transform"]}
					left={tS[item]["label"]["hoverLeft"]}
					top={tS[item]["label"]["top"]}
					opacity={tS[item]["label"]["opacity"]}
					labelDisplay={tS[item]["label"]["display"]}
					display={tS[item]["label"]["hoverDisplay"]}
					onContextMenu={(e) => {
						showContextMenu(e);
					}}
					onClick={(event) => {
		...
		let ancRanks: any[] = JSON.parse(
			JSON.stringify(this.state.ancestorRanks)
		).reverse();
		...
		return [
			...
			>
				{shapes} {labels}
				<clipPath key={"clipPath"} id="mask">
					{clipPaths}
				</clipPath>
			</svg>,

			<div key={"div-ancestors"} id="ancestors">
				{ancestors}
			</div>,

			<div key={"left-column"} id="left-column">
				<AncestorSection
					key={"ancestor-section"}
					ancestors={anc}
					root={this.state.root}
					layer={this.state.layer}
					plotEValue={this.state.plotEValue}
					ancestorRanks={ancRanks}
					onClickArray={anc.map((self, index) => (event) => {
						this.handleClick(`${self}_-_${-index}`, event);
					})}
				/>
				<DescendantSection
					key={"descendant-section"}
					self="Felinae"
					layer={0}
					ancestor="Felidae"
					hovered={true}
...
function TaxonShape(props) {
	return (
		<path
			id={props.id}
			data-taxname={props.taxName}
			data-taxrank={props.taxRank}
			label-display={props.labelDisplay}
			className="hoverable-object"
			d={props.d}
			onMouseOver={(e) =>
				hoverHandler(props.id, props.fullLabel, props.root, e)
			}
			onMouseOut={(e) => onMouseOutHandler(props.id, props.labelDisplay, e)}
