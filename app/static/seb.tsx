...
var allTaxa:object = JSON.parse(`{"Corynebacteriales 26%":{"lineageNames":[["superkingdom","Nanhermannia nana"],["kingdom","Antibiotic-producing bacteria"],["phylum","Corynebacteriales 26%"]],"rank":"phylum","taxID":"0","totalCount":26,"unassignedCount":26},
...
class PlotDrawing extends React.Component<{lineages:string[][], ranks:string[][]}, {root:string, layer:number, collapse:boolean, horizontalShift:number, verticalShift:number, taxonSpecifics:object, croppedLineages:string[][], alignedCroppedLineages:string[][], croppedRanks:string[][], unassignedCounts:string[][], structureByDegrees:object, structureByTaxon: object, svgPaths:object, shapeComponents:object, shapeCenters:object, taxonLabels:object, taxonShapes:object, colors:string[], ancestors:string[], rankPattern:string[], alteration:string, totalUnassignedCount:number, numberOfLayers:number, layerWidth:number, count:number}> {
    ...
    calculateSVGPaths(newState:object):void {
        ...
        var layerWidth:number = Math.max((smallerDimension - dpmm * 25) / numberOfLayers, dpmm * 4);
    ...
    calculateTaxonLabels(newState:object):void {
        ...
        var numberOfLayers:number = alignedCroppedLineages[0].length;
        var smallerDimension:number = Math.min(this.state.horizontalShift, this.state.verticalShift);
        var layerWidthInPx:number = Math.max((smallerDimension - dpmm * 25) / numberOfLayers, dpmm * 4);
        var startDeg = (key) => {return taxonSpecifics[key]["degrees"][0]};
        var endDeg = (key) => {return taxonSpecifics[key]["degrees"][taxonSpecifics[key]["degrees"].length-1]};
    ...
    render() {
        ...
        return [<svg style={{"height": "100%", "width": "100%", "margin": "0", "padding": "0", "boxSizing": "border-box", "border": "none", backgroundColor: "white"}} id="shapes">{shapes} <clipPath id="mask">{clipPaths}</clipPath></svg>,<div id="labels">{labels}</div>]
    }
}
...
function TaxonShape(props) {
    return <path id={props.id} d={props.d} onClick={props.onClick} style={{"stroke": props.stroke, "strokeWidth": "0.2vmin", "fill": props.fillColor}}/>;
}
function TaxonLabel(props) {
    return <p id={`${props.id}-label`} onClick={props.onClick} style={{"margin": "0", "paddingLeft": "2px", "paddingRight":"2px", "position": "absolute", "fontFamily": fontFamily, "fontSize": "2vmin", "fontWeight": "bold", "left": props.left, "right": props.right, "top": props.top, "transformOrigin": props.transformOrigin, "transform": props.transform, "color": "white", "opacity": props.opacity, "display": props.display, "backgroundColor": "black", "border": "0.2vmin solid black"}}>{props.abbr}</p>
}
