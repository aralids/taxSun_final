...
var PlotDrawing = /** @class */ (function (_super) {
    ...
    PlotDrawing.prototype.getStructureByTaxon = function (newState, bypassReducing) {
        ...
        for (var i = 1; i < layersUnique.length; i++) {
            for (var j = 0; j < layersUnique[i].length; j++) {
                ...
                var degreeSections = Object.keys(newState["structureByDegrees"]);
                var startDegrees = Number(degreeSections[firstOccurrenceInLayer].slice(0, degreeSections[firstOccurrenceInLayer].length - 4).split("-")[0]);
                var endDegrees = Number(degreeSections[lastOccurrenceInLayer].slice(0, degreeSections[lastOccurrenceInLayer].length - 4).split("-")[1]);
                var actualIndex = i;
                var verticalMin = i;
                ...
                for (var l = firstOccurrenceInLayer; l <= lastOccurrenceInLayer; l++) {
                    var point = Number(degreeSections[l].slice(0, degreeSections[l].length - 4).split("-")[1]);
                    var layer = i + 1;
    ...
    PlotDrawing.prototype.calculateTaxonLabels = function (newState) {
        ...
        for (var _i = 0, _a = Object.keys(structureByTaxon); _i < _a.length; _i++) {
            ...
            var leftBorder = newState["shapeComponents"][key]["leftBorder"];
    ...
    PlotDrawing.prototype.checkTaxonLabelWidth = function () {
        var taxonLabels = this.state.taxonLabels;
        var tooWide = [];
        for (var _i = 0, _a = Object.keys(taxonLabels); _i < _a.length; _i++) {
            var key = _a[_i];
            var height = document.getElementById(key + "-label").offsetHeight;
            var width = document.getElementById(key + "-label").offsetWidth;
...
function getPointAtLength(length, x1, y1, x2, y2) {
    var d = getLineLength(x1, y1, x2, y2);
    var t = length / d;
...
var lineagesNames = [];
var lineagesRanks = [];
for (var _k = 0, lineagesFull_1 = lineagesFull; _k < lineagesFull_1.length; _k++) {
    var lineage = lineagesFull_1[_k];
